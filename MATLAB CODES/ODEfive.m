function dydt = ODEfivevision(t,y)
%System of 5 equations  NON DIMENSIONAL
%   y(1) = zita1, dydt(1) = dzita1/dt
%   y(2) = zita2, dydt(2) = dzita2/dt
%   y(3) = delta, dydt(3) = ddelta/dt
%   y(4) = theta_f1, dydt(5) = dtheta_f1/dt
%   y(5) = theta_f2, dydt(6) = dtheta_f2/dt

% This script containes all the governing equations which are going to be
% integrated by ode113 in the main script fivesimulations.m 
...
%% A-DIPOLE
% dydt(4)=(1/16).*exp(1).^((sqrt(-1)*(-1)).*(2.*thetaf1+thetaf2)).*(128.*exp(1).^( ...
...
%   sin(thetaf1).^2.*tan(pi.*zita1))+((-1)+(-1).*cos(thetaf1+atan(delta.^(-1).*(zita1+(-1).*zita2)))).*(kv.*sin(thetaf1+( ...
%   -1).*thetaf2)+kp.*(delta.^2+(zita1+(-1).*zita2).^2).^(1/2).*sin(thetaf1+atan(delta.^(-1) ...
%   .*(zita1+(-1).*zita2))))+ks*sin(thetaf1);
% 
% dydt(5)=(1/8).*exp(1).^((sqrt(-1)*(-1)).*(thetaf1+thetaf2)).*(64.*exp(1).^(sqrt( ...
...
%   rho.^2.*psi.*sec(pi.*zita2).^2.*sin(thetaf2).^2.*tan(pi.*zita2))+(1+(-1).*cos(thetaf2+atan(delta.^(-1).*(zita1+(-1).*zita2)))).*(kv.*psi.*sin(thetaf1+( ...
%   -1).*thetaf2)+kp.*(delta.^2+(zita1+(-1).*zita2).^2).^(1/2).*sin(thetaf2+atan(delta.^(-1) ...
%   .*(zita1+(-1).*zita2))))+ks*psi*sin(thetaf2);
